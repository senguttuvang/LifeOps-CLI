...
import {
  ...
  VisionEventExtractionServiceLive,
} from "../../domain/whatsapp/services/vision-event-extraction.service";
 ...
 */
export const extractVisionEventsCommand = Command.make("extract-vision-events", {}, () =>
  Effect.gen(function* () {
    // Feature flag check
    ...
    // Sort by confidence and date (copy to mutable array first)
...
${top10.map((event: { eventName: string; date?: string; time?: string; location?: string; organizer?: string; category: string; confidence: number; description?: string; registrationInfo?: string; contactInfo?: string; sourceMessageId: string; sourceImagePath: string; caption?: string }, idx: number) => formatVisionEvent(event, idx + 1)).join("\n")}
