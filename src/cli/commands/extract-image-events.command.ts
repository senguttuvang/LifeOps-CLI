...
import {
  ...
  ImageEventExtractionServiceLive,
} from "../../domain/whatsapp/services/image-event-extraction.service";
 ...
 */
export const extractImageEventsCommand = Command.make("extract-image-events", {}, () =>
  Effect.gen(function* () {
    // Feature flag check
...
${top10.map((event: { eventName: string; date?: string; time?: string; location?: string; organizer?: string; category: string; confidence: number; description?: string; registrationInfo?: string; contactInfo?: string; sourceMessageId: string; sourceImageUrl: string; caption?: string }, idx: number) => formatImageEvent(event, idx + 1)).join("\n")}
