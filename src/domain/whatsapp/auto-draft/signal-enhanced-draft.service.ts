      ...
      Effect.gen(function* () {
        ...
        try {
          if (forceSignalExtraction) {
            if (verbose) {
              console.log(`[SignalDraft] Forcing signal extraction...`);
            }
            signals = yield* signalService.extractSignals(userId);
          } else {
            if (verbose) {
              console.log(`[SignalDraft] Loading cached signals...`);
            }
            ...
            if (!signals) {
              if (verbose) {
                console.log(`[SignalDraft] No signals found, extracting...`);
              }
              try {
                signals = yield* signalService.extractSignals(userId);
              } catch {
          ...
          if (verbose) {
            console.log(`[SignalDraft] Could not load signals, using basic RAG`);
          }
          signals = undefined;
        }
        ...
        if (verbose) {
          console.log(`[SignalDraft] Searching RAG for similar messages...`);
        }
        ...
        if (verbose) {
          console.log(`[SignalDraft] Found ${ragExamples.length} similar examples`);
        }
        ...
        if (verbose) {
          console.log(`[SignalDraft] Generating draft with LLM...`);
        }
        ...
        if (signals) {
          if (verbose) {
            console.log(`[SignalDraft] Enforcing signals on draft...`);
          }
