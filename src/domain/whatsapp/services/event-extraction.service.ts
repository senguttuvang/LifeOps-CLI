...
export class EventExtractionService extends Context.Tag("EventExtractionService")<
  EventExtractionService,
  {
    ...
    readonly extractEvents: (
      messages: ReadonlyArray<EventCandidate>,
    ) => Effect.Effect<ReadonlyArray<ExtractedEvent>, ClaudeError>;
    readonly deduplicateEvents: (
      events: ReadonlyArray<ExtractedEvent>,
    ) => Effect.Effect<ReadonlyArray<ExtractedEvent>, never>;
  }
>() {}
...
export const EventExtractionServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return {
       ...
       */
      findEventCandidates: (days: number) =>
        Effect.gen(function* () {
       ...
       */
      extractEvents: (messages: ReadonlyArray<EventCandidate>) =>
        Effect.gen(function* () {
          ...
          const messageBatch = messages
            .slice(0, 50)
            .map((m, idx) => `[${idx}] ${new Date(m.timestamp * 1000).toISOString().split("T")[0]}: ${m.content}`)
            .join("\n\n");
...
- Return empty array [] if no events found`;
       ...
       */
      deduplicateEvents: (events: ReadonlyArray<ExtractedEvent>) =>
        Effect.gen(function* () {
          ...
          for (const event of events) {
            ...
            if (!seen.has(key)) {
              ...
              if (existing) {
                (existing as any).sourceMessages = [...existing.sourceMessages, ...event.sourceMessages];
