...
export class ImageEventExtractionService extends Context.Tag("ImageEventExtractionService")<
  ImageEventExtractionService,
  {
    ...
    readonly extractEventsFromImages: (
      messages: ReadonlyArray<ImageMessage>,
    ) => Effect.Effect<ReadonlyArray<ImageExtractedEvent>, ClaudeError>;
  }
>() {}
...
export const ImageEventExtractionServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return {
       ...
       */
      extractEventFromImage: (message: ImageMessage) =>
        Effect.gen(function* () {
          if (!message.caption) {
            return null;
          }
...
- "Hello Everyone." → return null (not an event)`;
       ...
       */
      extractEventsFromImages: (messages: ReadonlyArray<ImageMessage>) =>
        Effect.gen(function* () {
